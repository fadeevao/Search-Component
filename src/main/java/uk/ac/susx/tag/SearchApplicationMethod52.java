...
public class SearchApplicationMethod52 {
    ...
    public List<Datum> integrationWithNeighbourFilePresent(List<Datum> objectsToSearch, String idKey, String messageIdKey,  String indexDirectoryPath, String searchTerm, String neighbourFilePath) throws IOException, InvalidTokenOffsetsException, ParseException, IndexingException {
    ...
    public List<Datum> integrationWithoutNeighbourFilePresent(List<Datum> objectsToSearch, String idKey, String messageIdKey, String indexDirectoryPath, String searchTerm) throws IOException, InvalidTokenOffsetsException, ParseException, IndexingException {
        Indexer indexer = new Indexer(indexDirectoryPath); //can also pass in a directory object in the constructor (normally RAMDirectory) but that's not efficient for big collections of data
        indexer.createIndex(objectsToSearch, idKey, messageIdKey);
        Searcher searcher = new Searcher(indexDirectoryPath); //if creating a RAMDirectory, then pass it into the Searcher's constructor
